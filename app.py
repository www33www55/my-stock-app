...
st.set_page_config(page_title=" 全池掃描", layout="wide"

st.title("🔥 未來小股神 10.2｜全池 AI 選股器")
st.caption("全上市櫃掃描｜技術分＋法人分＋主力分｜自動票選排行榜")

st.warning("這是選股輔助，不是買賣保證。")
...
def get_inst(stock_id):
    ...
    return {
        ...
        "三大法人": int(total),
        "法人分": min(score, 100),
        "法人原因": "、".join(reason) if reason else "法人普通"
    }
...
def tech_score(df):
    ...
    if mm.iloc[-1] > ss.iloc[-1] and mm.iloc[-1] > 0:
        score += 15
        reason.append("MACD主升")

    if hh.iloc[-1] > hh.iloc[-2]:
        score += 6
        reason.append("MACD動能增強")
    ...
    return {
        ...
        "成交量": volume,
        "技術分": min(score, 100),
        "RSI": round(float(rr.iloc[-1]), 1),
        "MACD": round(float(mm.iloc[-1]), 2),
        "技術原因": "、".join(reason) if reason else "技術普通"
    }
...
def main_score(tech, inst):
    ...
    return {
        "主力分": min(score, 100),
        "主力原因": "、".join(reason) if reason else "主力普通"
    }
...
if st.button("🚀 開始全池掃描", use_container_width=True):
    ...
    if result.empty:
        ...
        st.subheader("🔥 五星強勢")
        df1 = result[result["總分"] >= 88]
        st.dataframe(df1[show_cols], use_container_width=True, hide_index=True)

        st.subheader("⭐ N字第二波")
        df2 = result[result["技術原因"].str.contains("N字第二波", na=False)]
        st.dataframe(df2[show_cols], use_container_width=True, hide_index=True)

        st.subheader("🚀 平台突破")
        df3 = result[result["技術原因"].str.contains("平台突破", na=False)]
        st.dataframe(df3[show_cols], use_container_width=True, hide_index=True)
        ...
        st.dataframe(df4[show_cols], use_container_width=True, hide_index=True)
