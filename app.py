...
st.title("🔍 全能技術分析健檢儀表板 (終極完全體 + KD背離偵測)")
...
try:
    ...
    if not df.empty:
        ...
        kd_bear_div = False # 高檔背離
        kd_bull_div = False # 低檔背離
        ...
        # 顯示綜合評分與即時警報雷達 (加入背離判斷)
        if kd_bear_div:
            score = min(score, 20)
            st.markdown(f"### 🎯 技術面綜合多頭評分：` {score} / 100 ` 分")
            st.markdown(f"### 🚦 今日即時訊號：🚨🚨 **特大警報！偵測到「KD 高檔背離」！股價拉高但動能不足，極度容易暴跌，千萬不要買！** 🚨🚨")
        elif is_overheated:
            score = min(score, 40)  
            st.markdown(f"### 🎯 技術面綜合多頭評分：` {score} / 100 ` 分")
            ...
            if kd_bull_div and latest['KD_Cross']:
                st.markdown("### 🚦 今日即時訊號：🌟🚀 **黃金訊號：偵測到「KD 低檔背離」+ 黃金交叉！這是主力默默吃貨的絕對底部！** 🚀🌟")
                ...
                st.markdown("### 🚦 今日即時訊號：🍏 **多頭趨勢良好，目前在等KD拉回低檔，請續抱。**")
            else:
                st.markdown("### 🚦 今日即時訊號：❌ **目前處於弱勢調整期，不符合買點。**")
